...
class LSTMCell(nn.Module):
    ...
    def forward(self, input_, hx):
        ...
        print(input_.shape, self.weight_ih.shape, wh_b.shape)
...
class LSTM(nn.Module):
    ...
    def forward(self, input_, length=None, hx=None):
        ...
        if hx is None:
            hx = (Variable(nn.init.xavier_uniform(torch.zeros((self.num_layers, batch_size, self.hidden_size)).to(input_.device))),
                  Variable(nn.init.xavier_uniform(torch.zeros((self.num_layers, batch_size, self.hidden_size)).to(input_.device))))
        h_n = []
        c_n = []
        layer_output = None
        for layer in range(self.num_layers):
